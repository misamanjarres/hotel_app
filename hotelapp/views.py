...
def getHotel(request):
    ...
    #hotels=Hotel.objects.all().to_json()
    #jsonString=json.loads(hotels)
...
def getPopulation(request):
    if request.method == 'POST':
        ...
        #return JsonResponse(populationjson, safe=False)
        #c = RequestContext(request,{'populationjson':populationjson})

        #return render(request,'map.html',{"populationjson":populationjson}, context_instance=RequestContext(request))
